...
(function () {
  ...
  function showSettingsPopup() {
    ...
    ensureTranslationsLoaded(false)
      ...
      .finally(function () {
        ...
        modal.style.cssText = `position:relative;background:${colors.modalBg};color:${colors.text};border:2px solid ${colors.border};border-radius:12px;width:460px;padding:20px 24px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px ${colors.shadowRgba};animation:slideUp 0.1s ease-out;`;
        ...
        function createMenuButton(id, key, fallback, iconClass, isPrimary) {
          ...
          // Skip container append, we'll append to advancedButtons later if needed
          // Wait, removeBtn and fixesMenuBtn still need to be appended.
          // I'll leave the append inside createMenuButton but check types.
          if (id !== "lt-settings-check" && id !== "lt-settings-fetch-apis") {
        ...
        const manageGameLabel = createSectionLabel(
          "menu.manageGameLabel",
          "Manage Game",
        );
        if (!isGamePage) manageGameLabel.style.display = "none";
        ...
        if (!isGamePage) fixesMenuBtn.style.display = "none";

        createSectionLabel("menu.advancedLabel", "Advanced");
        const advancedButtons = document.createElement("div");
        advancedButtons.style.cssText = "display:flex;gap:10px;";
        ...
        checkBtn.style.padding = "12px 10px";
        ...
        fetchApisBtn.style.flex = "1";
        fetchApisBtn.style.fontSize = "13px";
        fetchApisBtn.style.padding = "12px 10px";

        advancedButtons.appendChild(checkBtn);
        advancedButtons.appendChild(fetchApisBtn);
        container.appendChild(advancedButtons);
  ...
  function showSettingsManagerPopup(forceRefresh, onBack) {
    ...
    function renderSettings() {
      ...
      for (let i = 0; i < state.config.schema.length; i++) {
        ...
        for (let j = 0; j < options.length; j++) {
          ...
          optionLabel.style.cssText = `font-size:16px;font-weight:600;color:${optLabelColors.text};`;
          ...
          if (option.description) {
            ...
            optionDesc.style.cssText = `margin-top:2px;font-size:12px;color:${optDescColors.textSecondary};line-height:1.4;`;
          ...
          if (option.type === "toggle") {
            ...
            optionEl.appendChild(toggleLabel);
          } else {
            optionEl.appendChild(labelWrap);
            ...
            if (isRightAligned) {
              ...
              controlWrap.style.cssText =
                "width: 180px; flex-shrink: 0; display: flex; justify-content: flex-end;";
            } else {
    ...
    function createFixListItem(fix, container) {
      ...
      itemEl.style.cssText = `margin-bottom:14px;padding:16px 20px;background:rgba(${itemColors.rgbString},0.03);border:1px solid ${itemColors.border};border-left:4px solid ${accentColor};border-radius:8px;display:flex;justify-content:space-between;align-items:center;transition:all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);box-shadow:0 2px 8px rgba(0,0,0,0.1);`;
      ...
      itemEl.onmouseout = function () {
        const c = getThemeColors();
        this.style.borderColor = c.border;
        this.style.background = `rgba(${c.rgbString},0.03)`;
        this.style.transform = "translateY(0)";
      ...
      gameName.style.cssText = `font-size:17px;font-weight:700;color:${nameColors.text};margin-bottom:4px;letter-spacing:0.3px;`;
      ...
      detailsDiv.style.cssText = `font-size:13px;color:${detailsColors.textSecondary};display:flex;flex-wrap:wrap;gap:12px;opacity:0.85;`;

      if (fix.fixType) {
        const typeSpan = document.createElement("div");
        const typeColors = getThemeColors();
        typeSpan.innerHTML = `<i class="fa-solid fa-layer-group" style="margin-right:5px;color:${typeColors.accent};opacity:0.7;"></i>${fix.fixType}`;
    ...
    function createLuaListItem(script, container) {
      ...
      const accentColor = itemLuaColors.accent || "#1a9fff";
      itemEl.style.cssText = `margin-bottom:14px;padding:16px 20px;background:rgba(${itemLuaColors.rgbString},0.03);border:1px solid ${itemLuaColors.border};border-left:4px solid ${accentColor};border-radius:8px;display:flex;justify-content:space-between;align-items:center;transition:all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);box-shadow:0 2px 8px rgba(0,0,0,0.1);`;
      ...
      gameName.style.cssText = `font-size:17px;font-weight:700;color:${gameNameLuaColors.text};margin-bottom:4px;letter-spacing:0.3px;display:flex;align-items:center;flex-wrap:wrap;`;
      gameName.textContent = gameNameText;
      ...
      detailsDiv.style.cssText = `font-size:13px;color:${detailsLuaColors.textSecondary};display:flex;flex-wrap:wrap;gap:12px;opacity:0.85;`;

      if (script.modifiedDate) {
        const dateSpan = document.createElement("div");
        const dateLuaColors = getThemeColors();
        dateSpan.innerHTML = `<i class="fa-solid fa-pen-to-square" style="margin-right:5px;color:${dateLuaColors.accent};opacity:0.7;"></i><strong style="font-weight:500;">${t("settings.installedLua.modified", "Modified:")}</strong> ${script.modifiedDate}`;
        detailsDiv.appendChild(dateSpan);
      }
  ...
  function showMillenniumDisclaimerModal() {
    ...
    overlay.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);z-index:100005;display:flex;align-items:center;justify-content:center;";

    const modal = document.createElement("div");
    modal.style.cssText =
      "background:linear-gradient(180deg,#5a4100,#362600);color:#fff;border:2px solid rgba(255,180,60,0.9);border-radius:12px;width:580px;padding:36px;box-shadow:0 25px 70px rgba(0,0,0,.9);animation:slideUp 0.2s ease-out;";
    ...
    icon.className = "fa-solid fa-triangle-exclamation";
    icon.style.cssText =
      "color:#FFE1A8;font-size:48px;filter:drop-shadow(0 0 10px rgba(255,225,168,0.5));";
    iconContainer.appendChild(icon);

    const titleEl = document.createElement("div");
    titleEl.style.cssText =
      "font-size:24px;font-weight:800;text-align:center;margin-bottom:24px;color:#FFE1A8;letter-spacing:0.5px;";
    titleEl.textContent = t("disclaimer.title", "Security & Support Notice");

    const messageEl = document.createElement("div");
    messageEl.style.cssText =
      "font-size:15px;line-height:1.6;margin-bottom:28px;color:#ffecec;text-align:center;";

    const line1 = document.createElement("div");
    line1.style.cssText = "margin-bottom:12px;font-weight:600;";
    line1.textContent = t(
      "disclaimer.line1",
      "LuaTools is not affiliated in any way with Millennium",
    );
    ...
    line2.textContent = t(
      "disclaimer.line2",
      "Millennium will NOT offer you support for this plugin on their discord server",
    );
    ...
    line3.textContent = t(
      "disclaimer.line3",
      "You will be BANNED from both LuaTools and Millennium servers if you go to their discord asking for help",
    );
    ...
    inputLabel.style.cssText =
      "font-size:12px;color:#8f98a0;margin-bottom:10px;text-align:center;text-transform:uppercase;letter-spacing:1px;";
    ...
    input.style.cssText =
      "width:100%;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.1);border-radius:6px;padding:12px;color:#fff;font-size:14px;outline:none;text-align:center;transition:all 0.3s ease;";
            ...
            function showSourceSelectionModal(appid, available) {
              ...
              if (apiList) {
                ...
                available.forEach((source) => {
                  ...
                  btn.style.cssText = `display:flex; align-items:center; justify-content:center; padding:12px; background:rgba(${colors.rgbString},0.1); border:1px solid ${colors.borderRgba}; border-radius:8px; text-decoration:none; transition:all 0.2s ease;`;

                  const name = document.createElement("div");
                  name.style.cssText = `font-size:15px; font-weight:600; color:${colors.text};`;
          ...
          try {
            ...
            if (status) {
              ...
              if (st.status === "checking content")
                status.textContent = lt("Checking content…");
            ...
            if (st.status === "done") {
              ...
              if (status) {
                ...
                const workshopText = workshopRaw
                  .replace(/[\u2705\u274C\uD83C\uDF89]/g, "")
                  .trim();
                const workshopIncluded =
                  workshopRaw.includes("🎉") || workshopRaw.includes("✅");
                const workshopBg = workshopIncluded
                  ? `rgba(${colors.rgbString},0.2)`
                  : "rgba(255,0,0,0.15)";
                const workshopBorder = workshopIncluded
                  ? colors.accent
                  : "#ff5c5c";
                const workshopColor = workshopIncluded
                  ? colors.accent
                  : "#ff5c5c";
                ...
                workshopItem.innerHTML = `<div style="font-size:14px;color:${
                  colors.textSecondary
                };font-weight:500;">${lt(
                  "Workshop: ",
                )}</div><div style="font-size:14px;color:${workshopColor};display:flex;align-items:center;gap:6px;"><span>${workshopText}</span><i class="fa-solid ${workshopIcon}" style="color:${workshopColor};"></i></div>`;
                status.appendChild(workshopItem);

                // DLC pills
                if (result.dlc.missing.length || result.dlc.included.length) {
                  const stripEmoji = function (s) {
                  ...
                  if (result.dlc.included.length > 0) {
                    ...
                    dlcIncItem.innerHTML = `<div style="font-size:14px;color:${
                      colors.textSecondary
                    };font-weight:500;">${lt("Dlc: ")}${
                      result.dlc.included.length
                    } ${stripEmoji(
                      lt("Included"),
                    )}</div><div style="font-size:14px;color:${
                      colors.accent
                    };display:flex;align-items:center;gap:6px;"><i class="fa-solid fa-check" style="color:${
                      colors.accent
                    };"></i></div>`;
                  ...
                  if (result.dlc.missing.length > 0) {
                    ...
                    dlcMissItem.innerHTML = `<div style="font-size:14px;color:${
                      colors.textSecondary
                    };font-weight:500;white-space:nowrap;">${lt("Dlc: ")}${
                      result.dlc.missing.length
                    } ${stripEmoji(
                      lt("Missing"),
                    )}</div><div style="font-size:14px;color:#ff5c5c;display:flex;align-items:center;gap:6px;text-align:right;word-break:break-word;"><span>(${dlcIds})</span><i class="fa-solid fa-xmark" style="color:#ff5c5c;flex-shrink:0;"></i></div>`;
